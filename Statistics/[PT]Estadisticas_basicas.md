...
### [1. Qual é a estatística?](#index)

Os seres humanos, como [outras espécies](https://pubmed.ncbi.nlm.nih.gov/35695157/) são capazes de reconhecer [padrões](https://pubmed.ncbi.nlm.nih.gov/ 7488849/ ) e obter informações sobre o mundo ao seu redor. A interpretação dessas observações nem sempre foi a mais próxima da realidade, mas assumimos a responsabilidade de ajustar nossas observações, análises e interpretações para obter melhor previsibilidade nos eventos que ocorrem ao nosso redor. É aí que a estatística nos fornece a metodologia adequada para recolher, analisar, interpretar e tirar conclusões a partir da informação que recolhemos dos diferentes eventos observados.

Ele também nos fornece métodos para, entre outras coisas:

1. Design: Planejamento e realização de estudos de pesquisa.
...
`População` e `amostra` são dois conceitos básicos de estatística. A **`população`** pode ser definida como o conjunto de pessoas, o universo de possíveis observações ou objetos sobre os quais se concentra uma investigação. Considerando que, **`amostra`** é chamado de **subconjunto** de indivíduos, objetos ou eventos efetivamente estudados daquela população.


### [3. Distribuições](#distribuições)

Acontece que podemos aprender muito sobre como algo acontece, mesmo que não conheçamos o processo subjacente que o causa. Por exemplo, observar quais valores são comuns (frequentes) e quais são incomuns, e fazer previsões sobre os valores que esta ou aquela variável (característica) assumirá, mesmo quando não sabemos o porquê.
...
É comum ouvir **"a ciência afirma o que"**, mas não é tão comum ouvir explicações sobre como a ciência afirma.
...
Aqui deixamos alguns vídeos para começarmos a pensar juntos sobre o alcance e as implicações de como sabemos e como afirmamos em ciência de dados... 😜

  - [Conhecimento - Mentiras a Verdade](https://www.youtube.com/watch?v=v-Z9eMt-8UU&t=581s)
  - [As patovicas da ciência](https://www.youtube.com/watch?v=ZMUjO5N9BCo)
...
| Parâmetro | Valor|
| -----------| -------------|
| Total de rãs medido | 500.000|
| média | 5.033|
| Mediana | 5.031|

>
> Que distribuição segue esta variável? Como você percebeu?
>
...
Existem aquelas conhecidas como **`variáveis ​​quantitativas ou numéricas`**, que, como o próprio nome diz, assumem valores numéricos. Estes podem ser:
   - Discreto: pode assumir um número contável/finito de valores. Por exemplo, os resultados do lançamento de um dado.

   - Contínuo: que pode assumir um valor fixo dentro de um determinado intervalo. Por exemplo, os pesos dos sapos carecas em Marte 🐸😜
...
Mas há também um tipo de variável não numérica ou **`qualitativa`**, que, como o próprio nome diz, dá conta de uma característica ou tributo que pode ser descrito em palavras ou `"rótulos"`. Estes por sua vez podem ser:
     ...
     - Ordinal: São aqueles que podem ser ordenados em uma hierarquia específica. Por exemplo, os resultados de uma pesquisa de satisfação com determinado serviço prestado aos clientes (muito insatisfeito, insatisfeito, neutro, satisfeito, muito satisfeito).
...
Mas como melhor do que dizer é fazer, agora que já sabemos um pouco mais sobre distribuições e estatísticas, vamos partir do conteúdo específico de [**Análisis Datos**](https://github.com/AJVelezRueda/Fundamentos_de_informatica /blob/master/Data_Science/Data_analysis_with_pandas.md).